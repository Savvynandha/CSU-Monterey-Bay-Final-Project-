    ...
    <table class="table table-hover">
        ...
        <%
            for (int i = starting_hour; i < ending_hour; i++) {
                ...
                ResultSet rs2 = stmt2.executeQuery("select * from booking_detail where starting_hour='" + i + "' and booking_id in (select booking_id from booking where sp_email=\'" + sp_email + "\'and date=\'" + date1 + "\')");
                if (rs2.next()) {
        %>
...
<input type="submit"  class="btn btn-success"  style="background: white;color:white"  value="Pay Now" onclick="return go3()"/>    

            <input type="submit"  class="btn btn-success" style="background: #3e3d3d;color:white"  value="Book" onclick="return go4()"/>    
        </div>
        </center>
