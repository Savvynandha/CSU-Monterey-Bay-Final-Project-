...
<%------------------------------ the code below is not original ----------------------------------%>
<%-- 
    Document   : fetch_reviews
    Created on : 5 May, 2017, 4:33:01 PM
    Author     : SONY
--%>
...
<html>
    ...
    <body>
        <%  String sp_email=request.getParameter("sp_email");
                ...
                Connection conn1 = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/mydb", "root", "komal");
                Statement stmt1 = conn1.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                ResultSet rs1 = stmt1.executeQuery("select * from rating");
                while(rs1.next())
                {
                    String review=rs1.getString("review");
                    int rating=rs1.getInt("num");
                    String user_email=rs1.getString("user_email");
                    Date datetime=rs1.getDate("datetime");
             ...
             <div class="row" style="margin-top: 20px;margin-left: 10px">
                 ...
                 {
                   %>
                   <img src="filled_star.png" style="height: 25px;width: 25px" />
                   <%
                 }
                  for(int i=1;i<=5-rating;i++)  
{                   
                 %>
                 <img src="empty_star.png" style="height: 25px;width: 25px" />
                 <%
                     }
                  %>
                  <label style="margin-left: 40px"><%=name%></label> <label style="margin-left: 700px"><%=datetime%></label><br>
                  <label style="margin-top:  20px"><%=review%></label>
